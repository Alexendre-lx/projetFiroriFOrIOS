...
// testfior
//
// Created by SAP Cloud Platform SDK for iOS Assistant application on 30/01/20
//
...
public class OnboardingFlowProvider: OnboardingFlowProviding {
    ...
    private func configuredWelcomeScreenStep() -> WelcomeScreenStep {
        let discoveryConfigurationTransformer = DiscoveryServiceConfigurationTransformer(applicationID: "fr.codi.fiori4ios", authenticationPath: "fiori4iosDestination")
        ...
        welcomeScreenStep.welcomeScreenCustomizationHandler = { welcomeStepUI in
            welcomeStepUI.headlineLabel.text = "Fiori for IOS"
            welcomeStepUI.detailLabel.text = NSLocalizedString("keyWelcomeScreenMessage", value: "Alexendre OBLI", comment: "XMSG: Message on WelcomeScreen")
            welcomeStepUI.primaryActionButton.titleLabel?.text = "Entrer"
            welcomeStepUI.footnoteActionButton.setTitle("", for: .normal)
            welcomeStepUI.footnoteLabel.text = ""
            ...
            if let welcomeScreen = welcomeStepUI as? FUIWelcomeScreen {
                ...
                welcomeScreen.emailTextField.text = "user@sap.com"
            }
        }
